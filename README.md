...
- Native-docker PR and CI workflows now require `DEV_AUTO_MYSQL_HOST`, `DEV_AUTO_MYSQL_TCP_PORT`, `DEV_AUTO_OVERRIDE`, and `DEV_E2E_MYSQL_TCP_PORT` alongside `LOG_BACKUP_COUNT`, `LOG_DIR`, `LOG_LEVEL_CONSOLE`, and `LOG_LEVEL_FILE`.
...
07. For native-docker PR workflows, define the required repository variables (`DEV_AUTO_MYSQL_HOST`, `DEV_AUTO_MYSQL_TCP_PORT`, `DEV_AUTO_OVERRIDE`, `DEV_E2E_MYSQL_TCP_PORT`, `RTEAPI_BASE_IMAGES_PATH`, `RTEAPI_ES_*`, `RTEAPI_LOG_REQUESTS`, `LOG_BACKUP_COUNT`, `LOG_DIR`, `LOG_LEVEL_CONSOLE`, and `LOG_LEVEL_FILE`) before running the workflow; `PYTEST_MYSQL_*` repository variables are no longer required for this path.
08. For native-docker CI workflows, provide matching `DEV_AUTO_*`, `DEV_E2E_MYSQL_TCP_PORT`, and `LOG_*` secrets alongside the existing installer and database credentials; `PYTEST_MYSQL_*` secrets are no longer required by the reusable workflow.
