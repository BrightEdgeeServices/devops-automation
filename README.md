...
- Native-docker PR and CI workflows now require `DEV_AUTO_MYSQL_HOST`, `DEV_AUTO_MYSQL_TCP_PORT`, `DEV_AUTO_OVERRIDE`, `DEV_E2E_MYSQL_TCP_PORT`, `UMS_SERVICE_USERID`, and `UMS_SERVICE_CLIENT_PASSWORD` alongside `LOG_BACKUP_COUNT`, `LOG_DIR`, `LOG_LEVEL_CONSOLE`, and `LOG_LEVEL_FILE`.
- The private native-docker PR template forwards `DEV_AUTO_MYSQL_HOST`, `DEV_AUTO_MYSQL_TCP_PORT`, `DEV_AUTO_OVERRIDE`, and `DEV_E2E_MYSQL_TCP_PORT` from repository variables into the reusable workflow.
...
07. For native-docker PR workflows, define the required repository variables (`DEV_AUTO_MYSQL_HOST`, `DEV_AUTO_MYSQL_TCP_PORT`, `DEV_AUTO_OVERRIDE`, `DEV_E2E_MYSQL_TCP_PORT`, `RTEAPI_BASE_IMAGES_PATH`, `RTEAPI_ES_*`, `RTEAPI_LOG_REQUESTS`, `LOG_BACKUP_COUNT`, `LOG_DIR`, `LOG_LEVEL_*`, `AUTHZ_SERVICE_USERID`, `ES_SERVICE_USERID`, `MYSQL_ROOT_USER`, and `UMS_SERVICE_USERID`) and provide the `AUTHZ_SERVICE_CLIENT_PASSWORD`, `ES_SERVICE_CLIENT_PASSWORD`, and `UMS_SERVICE_CLIENT_PASSWORD` secrets before running the workflow; `PYTEST_MYSQL_*` repository variables are no longer required for this path.
08. For native-docker CI workflows, provide matching `DEV_AUTO_*`, `DEV_E2E_MYSQL_TCP_PORT`, `LOG_*`, `AUTHZ_SERVICE_USERID`, `ES_SERVICE_USERID`, `MYSQL_ROOT_USER`, and `UMS_SERVICE_USERID` values alongside the existing installer and database credentials, plus the `AUTHZ_SERVICE_CLIENT_PASSWORD`, `ES_SERVICE_CLIENT_PASSWORD`, and `UMS_SERVICE_CLIENT_PASSWORD` secrets; `PYTEST_MYSQL_*` secrets are no longer required by the reusable workflow.
